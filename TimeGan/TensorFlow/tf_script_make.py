...
def main():
    ...
    # Make Data
    #load_model(path_output+'autoencoder.keras')
    synth.autoencoder=load_model(path_output+'autoencoder.keras')
    synth.embedder=load_model(path_output+'embedder.keras')
    synth.supervisor=load_model(path_output+'supervisor.keras')
    synth.discriminator_model=load_model(path_output+'discriminator_model.keras')
    synth.adversarial_supervised=load_model(path_output+'adversarial_supervised.keras')
    synth.adversarial_embedded=load_model(path_output+'adversarial_embedded.keras')
    synth.generator=load_model(path_output+'generator.keras')

    synth.discriminator_model.summary()
    synth.autoencoder.summary()
    synth.supervisor.summary()
    synth.adversarial_supervised.summary()
    synth.adversarial_embedded.summary()
    synth.generator.summary()
    # Generate dataset
    #sample_size = 1152
    ...
    real_sample_=real_sample.reshape(-1,6)
    synthetic_sample_=synthetic_sample.reshape(-1,6) 

    np.savetxt(path_output+"real_sample.csv",real_sample_,delimiter=",")
    np.savetxt(path_output+"synthetic_sample.csv",synthetic_sample_,delimiter=",")
    np.save(path_output+"synthetic_sample",synthetic_sample)
    np.save(path_output+"real_samplen",real_sample)
