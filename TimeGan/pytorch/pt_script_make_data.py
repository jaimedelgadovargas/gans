...
def main():
    ...
    '''
    synth.embedder = (torch.load(path_output+"embedder.pt", weights_only=False))
    synth.recovery = (torch.load(path_output+"recovery.pt", weights_only=False))
    synth.generator = (torch.load(path_output+"generator.pt", weights_only=False))
    synth.supervisor = (torch.load(path_output+"supervisor.pt", weights_only=False))
    synth.discriminator = (torch.load(path_output+"discriminator.pt", weights_only=False))

    #torch.load(synth.embedder,path_output+"embedder.pt")
    #torch.load(synth.recovery,path_output+"recovery.pt")
    #torch.load(synth.generator,path_output+"generator.pt")
    #torch.load(synth.supervisor,path_output+"supervisor.pt")
    #torch.load(synth.discriminator,path_output+"discriminator.pt")
    ...
    #sample_size = 1000#250

    #print(len(stock_data))
    synth_data = synth.sample(len(stock_data))
    synthetic_sample = np.asarray(synth_data)
    real_shape = real_sample.shape
    synth_shape = synth_data.shape
    
    list=[]
    s = (24,6)
    temp_v=np.zeros(s)
    for i in range(0,int(synth_shape[0]/24-1)):
        for j in range(0,24):            
            k=i*24+j
            temp_v[j,:]=synth_data[k,:]
            
        list.append(temp_v)
    synthetic_sample=np.asarray(list)
    #synth_data.reshape(1,real_shape[1],real_shape[2])
    
    print(real_shape)    
    print(synth_data.shape)
    print(synthetic_sample.shape)

    real_sample_=real_sample.reshape(-1,n_seq)

    np.savetxt(path_output+"real_sample.csv",real_sample_,delimiter=",")
    np.savetxt(path_output+"synthetic_sample.csv",synth_data,delimiter=",")
